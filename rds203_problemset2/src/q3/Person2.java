...
public class Person2 {
    ...
    public void simulateYear(Parameters2 params, Random rand) {
    	
        if (state.equals("Dead")) return; // No changes if already dead

        double p_bg = params.getBackgroundMortality(age);
        
        // Ensure probabilities sum to <= 1 by normalising if needed
        if (state.equals("Well")) {
            ...
            // If combined probability exceeds 1, normalise them to maintain their relative risks
            ...
            // Draw a random number to determine outcome
